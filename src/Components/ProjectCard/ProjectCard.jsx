...
const ProjectCard = ({ data, idx }) => {
    const { name, description, technologies, githubRepo, liveDemo, cardImage, id } = data;

    // flex direction based on index
    const flexDirection = idx % 2 === 0 ? "lg:flex-row" : "lg:flex-row-reverse";

    return (
        <div className={` my-10 md:my-12 lg:my-20 2xl:my-28 flex flex-col items-center lg:items-end gap-5  ${flexDirection}`}>
            ...
            <div className='w-full lg:w-1/2'>
                ...
                <motion.div
                    ...
                    viewport={{ once: true }}
                >
                    <h1 className='montserrat font-bold uppercase text-3xl 2xl:text-5xl text-[#424874]'>{name}</h1>
                    <motion.a
                        href={liveDemo}
                        target='_blank'
